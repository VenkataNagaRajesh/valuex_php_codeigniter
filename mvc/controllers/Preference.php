...
class Preference extends Admin_Controller {
	...
	function server_processing(){		
		...
		$sQuery = "SELECT SQL_CALC_FOUND_ROWS p.*,dd.aln_data_value category,dd1.alias type,dt1.alias valuetype,dt2.alias value FROM VX_preference p LEFT JOIN VX_data_defns dd ON dd.vx_aln_data_defnsID = p.categoryID LEFT JOIN VX_data_types dd1 ON dd1.vx_aln_data_typeID = p.pref_type LEFT JOIN VX_data_types dt1 ON dt1.vx_aln_data_typeID = p.pref_get_value_type LEFT JOIN VX_data_types dt2 ON dt2.vx_aln_data_typeID = p.pref_get_value
		$sWhere			
		$sOrder
		$sLimit	"; 
