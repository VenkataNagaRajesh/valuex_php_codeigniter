  		...
  		if($connected) {
			...
			$this->load->dbutil();
  			if ($this->dbutil->database_exists($this->db->database)) {
				...
				$dbdump_file_path = dirname($this->config->index_path) . '/dbdump.sql';
				$this->form_validation->set_message("database_unique", $dbdump_file_path);
				if ( file_exists( $dbdump_file_path )) {
					$dbdump = file_get_contents($dbdump_file_path);
					$this->load->model('install_m');
					$this->install_m->use_sql_string($dbdump);
					return TRUE;
				} else {
				 $this->form_validation->set_message("database_unique", "Dump Not Found.");
					return FALSE;
				}
/*
					    $id = uniqid();
