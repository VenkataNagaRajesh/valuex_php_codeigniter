...
class season_m extends MY_Model {
	...
	protected $_table_name = 'VX_season';
	...
	function get_seasons($array=NULL, $signal=FALSE) {
     ...
     function get_seasons_where($where=null,$wherein=null){
            $this->db->select('s.*,dd.aln_data_value airline_name,dd.code airline_code,dt1.vx_aln_data_typeID orig_type,dt1.alias orig_level,dt2.vx_aln_data_typeID dest_type,dt2.alias dest_level')->from('VX_season s');
	...
	function getSeasons_for_triggerrun($timestamp) {
		$sql = "SELECT * FROM VX_season WHERE modify_date >= ".$timestamp;
		$seasons = $this->install_m->run_query($sql);
        return $seasons;
	}
	
	function getMarketSeasons_for_triggerrun() {
		$sql = "SELECT * FROM VX_season WHERE ( ams_orig_levelID = 17 OR ams_dest_levelID = 17)";
		$seasons = $this->install_m->run_query($sql);
        return $seasons;
	}
...
// select CONCAT(day(FROM_UNIXTIME(ams_season_start_date)),'-',month(FROM_UNIXTIME(ams_season_start_date)),'-',year(FROM_UNIXTIME(ams_season_start_date))) as date , FROM_UNIXTIME(ams_season_start_date) , day(FROM_UNIXTIME(ams_season_start_date)) as day ,month(FROM_UNIXTIME(ams_season_start_date)) as mon from VX_aln_season where CONCAT(day(FROM_UNIXTIME(ams_season_start_date)),'-',month(FROM_UNIXTIME(ams_season_start_date)),'-',year(FROM_UNIXTIME(ams_season_start_date))) = '1-6-2019';
	...
	function getSeasonsList($airlineID = array()){
                $this->db->select('VX_aln_seasonID,season_name, ds.aln_data_value as global_season');
                $this->db->from('VX_season s');
		$this->db->join('VX_data_defns ds','ds.vx_aln_data_defnsID = s.global_seasonID AND ds.aln_data_typeID = 27','LEFT');
		...
		//print_r($this->db->last_query()); exit;
                ...
                foreach($result  as $season) {
                        $seasons[$season->VX_aln_seasonID]  = $season->season_name .  " - " . $season->global_season;
                }
                return $seasons;
	...
	function getSeasonNameByID($id) {
		...
		$this->db->select('season_name')->from('VX_season');
	...
	public function searchAirlineCode($string){
		$query = "SELECT CONCAT(`code`, '_', `aln_data_value`) search from VX_data_defns WHERE ";
	...
	public function getSeasonValues($type,$wherein){
     ...
     function get_global_seasons($where=null,$wherein=null){
            $this->db->select('dd.vx_aln_data_defnsID,dd.aln_data_value,dd.airlineID')->from('VX_data_defns dd');
	    $this->db->where('dd.aln_data_typeID',27);//Season type
            $query = $this->db->get();
            return $query->result();
        }
}
