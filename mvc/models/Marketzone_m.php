...
class Marketzone_m extends MY_Model {
	...
	protected $_table_name = 'VX_market_zone';
	...
	function get_marketzonename($id) {
		$query = $this->db->get_where('VX_market_zone',array('market_id'=>$id));
		return $query->row();
	}
...
on MainSet.market_id = SubSet.market_id WHERE MainSet.market_id =".$id;
    ...
    function get_marketzones_count() {
		$this->db->select('count(*) as cnt')->from('VX_market_zone');
                $query = $this->db->get();
	...
	function insert_marketzone($array) {
		 $this->db->insert("VX_market_zone",$array);
	...
	function getMarketzones_for_triggerrun($timestamp) {
		$sql = "SELECT market_id FROM VX_market_zone
...
(find_in_set(c.areaID, mz.amz_level_name) AND mz.amz_level_id  = 5) 
  ) LEFT OUTER JOIN VX_market_airport_map mapl on (find_in_set(mapl.market_id, mz.amz_level_name) AND mz.amz_level_id  = 17) LEFT OUTER JOIN  VX_master_data cc ON ((find_in_set(cc.countryID, mz.amz_incl_name) AND mz.amz_incl_id  = 2) OR
...
LEFT OUTER JOIN  VX_master_data ce ON ((find_in_set(ce.countryID, mz.amz_excl_name) AND mz.amz_excl_id  = 2) OR
...
group by mz.market_id";   
	...
	function getSubmarketsForList($list){
		...
		foreach ($list as $id ) {
                        ...
                        $sql = "SELECT market_id FROM VX_market_zone WHERE active = 1 AND
	...
	function getMarkets_for_triggerrun($timestamp) {
		$sql = "SELECT market_id FROM VX_market_zone WHERE modify_date >= ".$timestamp." AND active = 1 ";
		$marketzones = $this->install_m->run_query($sql);
	...
	function getALndataTypeValues($list){
		$query = "SELECT aln_data_value FROM  VX_data_defns WHERE vx_aln_data_defnsID in (". $list . ")";
	...
	function getAirportsList($parentID){
		 $query = "select vx_aln_data_defnsID FROM 
					( SELECT * from 
					( SELECT * from VX_data_defns  order by parentID, vx_aln_data_defnsID) 
					  VX_data_defns, (select @pv := ".$parentID.") 
	...
	function getParentsofAirport($airportID) {
		$query = "SELECT T2.vx_aln_data_defnsID 
		          FROM (
    				SELECT
        				@r AS _id,
        				(SELECT @r := parentID FROM VX_data_defns WHERE vx_aln_data_defnsID = _id) AS parentID,
        				@l := @l + 1 AS lvl
    				FROM
        				(SELECT @r := ".$airportID.", @l := 0) vars,
        			    	VX_data_defns m
    					WHERE @r <> 0) T1
				JOIN VX_data_defns T2
...
function getParentsofAirportByType($airportID,$id) {
                $query = "SELECT T2.vx_aln_data_defnsID , T2.aln_data_value, T2.code
                          FROM (
                                ...
                                FROM
                                        ...
                                        WHERE @r <> 0) T1
                                JOIN VX_data_defns T2
                                        ON T1._id = T2.VX_data_defnsID WHERE T2.aln_data_typeID = ".$id."
                                ORDER BY T1.lvl DESC";
	...
	function getMarketsForAirportID($airport_id) {
		...
		$this->db->select('market_name')->from('VX_market_zone m');
   ...
   function getAirportsForMarketsList($market_list) {
	...
	function getAirportsMarketData($airline_in = array()) { 
	  ...
	  $sql  =  "select  mz.market_name, group_concat(dd.code)   as airports
			...
			INNER JOIN VX_market_zone mz on (mz.market_id = map.market_id)  
			INNER JOIN  VX_data_defns dd on (dd.vx_aln_data_defnsID =  map.airport_id ) " ;
		if(!empty($airline_in)){
