...
class Airline_m extends MY_Model {
	...
	protected $_table_name = 'VX_data_defns';
	...
	public function getAirlineData($id){
		$this->db->select('dd.*,dd.aln_data_value airline_name,dd.code,u.name modify_by,group_concat( distinct ac.aln_data_value,"/",sc.aln_data_value) as aircraft_seat_capacity,GROUP_CONCAT(d.aln_data_value SEPARATOR ", ") flights,a.logo,a.video_links')->from('VX_data_defns dd');
        $this->db->join('VX_airline a','a.airlineID = dd.vx_aln_data_defnsID','LEFT');		
        $this->db->join('VX_data_defns d','d.parentID = dd.vx_aln_data_defnsID AND d.aln_data_typeID = 16','LEFT');
		$this->db->join('VX_airline_aircraft aa','aa.airlineID = dd.vx_aln_data_defnsID','LEFT');
		$this->db->join('VX_data_defns ac','ac.vx_aln_data_defnsID = aa.aircraftID AND ac.aln_data_typeID = 21','LEFT');
        $this->db->join('VX_data_defns sc','sc.parentID = aa.aircraftID AND sc.aln_data_typeID = 22','LEFT');		
	...
	public function getAirlinesData(){
		$this->db->select('dd.*,dd.aln_data_value airline_name,dd.code,u.name modify_by')->from('VX_data_defns dd');		
		$this->db->join('VX_user u','u.userID = dd.modify_userID','LEFT');	
        $this->db->where('dd.aln_data_typeID',12);		
		$query = $this->db->get();
		return $query->result();
	...
	public function getClientAirline($userID, $getrow=NULL){
		...
		//$this->db->join('VX_airline a','a.VX_aln_airlineID = c.airlineID','LEFT');
		$this->db->join('VX_client_airline ca','ca.clientID = c.VX_aln_clientID','LEFT');
		$this->db->join('VX_data_defns dd','dd.vx_aln_data_defnsID = ca.airlineID','LEFT');
	...
	public function checkAirline($airline){
		$check = $this->db->get_where('VX_data_defns',array('aln_data_typeID' => 12,'code' => $airline['code']))->row();
		if(!empty($check)){
			//$data->existed = 1;
	...
	public function updateSeatCapacity($seat_capacity,$seatID){
		...
		$this->db->update('VX_data_defns',array('aln_data_value'=>$seat_capacity));
	}
	...
	public function update_airlinelogo($data){
		...
		if(empty($airline)){
			...
			$this->db->insert('VX_airline',$data);
		} else {
			$data['modify_userID'] = $this->session->userdata('loginuserID');
			$data['modify_date'] = time();			
			...
			$this->db->where('airlineID',$airlineID);
			$this->db->update('VX_airline',$data);
		}
	}
