...
class Permission_m extends MY_Model {
	...
	public function get_modules_with_permission($roleID,$products = null) {		
		...
		$query = "Select p.name module,p1.permissionID,p1.name,p1.description,p1.productID, (case when (p2.roleID = $roleID) then 'yes' else 'no' end) as active From VX_permissions p1 LEFT JOIN VX_products p ON p.productID = p1.productID left join VX_permission_relationships p2 ON p1.permissionID = p2.permission_id and p2.roleID =$roleID";
		...
		$query .= ' ORDER BY p1.productID ASC,active DESC,p1.permissionID ASC ';
		//print_r($query); exit;		
	...
	public function delete_all_permission($roleID)
	{
