...
class Report_m extends MY_Model {
    ...
    public function get_report($airlineID,$from_date,$to_date,$type = 1,$bid_accepted,$bid_rejected){ 
      ...
      $query = "  select  SQL_CALC_FOUND_ROWS  
                        ...
                        MainSet.to_cabin, MainSet.bid_value  ,MainSet.weight  , SubSet.fqtv, MainSet.cash, MainSet.miles, MainSet.offer_status,
			SubSet.from_cabin_id, MainSet.upgrade_type, SubSet.boarding_point, SubSet.off_point, MainSet.bid_submit_date, MainSet.booking_status, SubSet.from_city_code, SubSet.to_city_code,SubSet.from_city_name, SubSet.to_city_name,MainSet.bid_avg, MainSet.rank, MainSet.bid_markup_val,SubSet.carrier_code
                ...
                FROM ( 
                                ...
                                tdef.cabin as to_cabin, oref.pnr_ref, bid.flight_number,bid.cash, bid.miles  , bid.upgrade_type,bs.aln_data_value as offer_status, bid_submit_date, pe.booking_status, rank,bid.weight
                                from  
                                        VX_offer oref 
                                        LEFT JOIN UP_bid bid on (bid.offer_id = oref.offer_id) 
    ...
    public function get_report1($airlineID,$from_date,$to_date,$type = 1,$bid_accepted,$bid_rejected){ 
      if($type == 2){
        $swhere = " WHERE bid_submit_date >= ".strtotime($from_date)." AND bid_submit_date <= ".strtotime($to_date);         
      } else {
        $dwhere = " AND pf1.dep_date >= ".strtotime($from_date)." AND pf1.dep_date <= ".strtotime($to_date);
      }
 ...
 MainSet.upgrade_type, SubSet.boarding_point, SubSet.off_point, MainSet.bid_submit_date, MainSet.booking_status, SubSet.from_city_name, SubSet.to_city_name,MainSet.bid_avg, MainSet.rank, MainSet.bid_markup_val,SubSet.carrier_code, MainSet.product_id, MainSet.bid_id,MainSet.productID,MainSet.weight
...
FROM ( 
            ...
            tdef.cabin as to_cabin, oref.pnr_ref, bid.flight_number,bid.cash, bid.miles  , bid.upgrade_type,bs.aln_data_value as offer_status, bid_submit_date, pe.booking_status, rank, pe.product_id, pf.dep_date as flight_date,bid.productID,bid.weight
            from  
                    UP_bid bid 
...
LEFT JOIN VX_airline_cabin_def tdef on (tdef.carrier = pf.carrier_code AND  tcab.alias = tdef.level ) 
   ...
   LEFT  JOIN (
                    select  flight_number,group_concat(distinct first_name, ' ' , last_name , ' fqtv: ' , fqtv SEPARATOR '<br>'  ) as p_list ,group_concat(distinct fqtv) as fqtv,
                            group_concat(distinct dep_date) as flight_date  ,
                            pnr_ref, 
                               fc.code as from_city, 
                      tc.code as to_city, from_city as boarding_point , to_city as off_point, 
                    fc.aln_data_value as from_city_name, tc.aln_data_value as to_city_name,
                    
                             car.code as carrier, pf1.carrier_code, pf1.dtpf_id, cab.aln_data_value  as from_cabin
