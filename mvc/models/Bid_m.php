...
class bid_m extends MY_Model {
 ...
 /*  function getPassengers($pnr_ref,$bidstatus = NULL){
      ...
      $this->db->join('UP_dtpf_ext pfe','(tpf.dtpf_id = pfe.dtpf_id )','LEFT');		 
	  $this->db->join('UP_fare_control_range fclr','(fclr.fclr_id = pfe.fclr_id AND fclr.from_cabin=tpf.cabin)','LEFT');	
   ...
   */
   ...
   function getPassengers($pnr_ref,$bidstatus = NULL){
	  $this->db->select("car.aln_data_value carrier_name,tpf.carrier_code carrier,tpf.cabin,tpf.pnr_ref,tpf.seat_no,def.desc current_cabin,group_concat(distinct fclr.fclr_id) fclr,group_concat(distinct fclr.to_cabin,'-',fclr.fclr_id,'-',pfe.booking_status,'-',tdef.level) to_cabins,group_concat( distinct round(fclr.min)) min,group_concat( distinct round(fclr.max)) max,group_concat( distinct round(fclr.average)) avg,group_concat( distinct round(fclr.slider_start)) slider_position,tpf.flight_number,tpf.dep_date, tpf.arrival_date,tpf.dept_time,tpf.arrival_time,car.code carrier_code,c1.aln_data_value from_city,c1.code from_city_code,c2.aln_data_value to_city,c2.code to_city_code,booking_status,group_concat(distinct pfe.dtpfext_id) as pf_list,group_concat( pax_contact_email) as email_list, group_concat(first_name,' ', last_name SEPARATOR ' ,') as pax_names,oref.miles,oref.offer_id,air_city1.aln_data_value air_from_city,air_city2.aln_data_value air_to_city,bid.bid_value,bid.miles bid_miles,bid.upgrade_type bid_upgrade")->from('VX_daily_tkt_pax_feed tpf');	
      $this->db->join('UP_dtpf_ext pfe','(tpf.dtpf_id = pfe.dtpf_id )','LEFT');		 
	  $this->db->join('UP_fare_control_range fclr','(fclr.fclr_id = pfe.fclr_id AND fclr.from_cabin=tpf.cabin)','LEFT');	
	 ...
	 // $this->db->join('vx_aln_data_defns  cab','(cab.vx_aln_data_defnsID = tpf.cabin AND cab.aln_data_typeID = 13)','LEFT');
	  ...
	  $this->db->join('UP_bid bid','(bid.offer_id = oref.offer_id AND tpf.flight_number = bid.flight_number)','LEFT');	
       $this->db->join('VX_data_defns dcto','(dcto.aln_data_typeID = 13 and dcto.vx_aln_data_defnsID = fclr.to_cabin)','INNER');
	  $this->db->join('VX_airline_cabin_def tdef','(tdef.carrier = tpf.carrier_code) AND (dcto.alias = tdef.level)','INNER');
  ...
  public function getPaxNames($pnr_ref){
	  $this->db->select("group_concat(first_name,' ', last_name SEPARATOR ' ,') as pax_names")->from('VX_daily_tkt_pax_feed');
  ...
  public function save_card_data($data){	  
	...
	public function getBidByOfferID($offer_id){
		$this->db->select('count(*) as count')->from('UP_bid');
  ...
  function bidsTotalCount(){
	...
	function getAirlineLogoByPNR($pnr_ref){
		$this->db->select("a.*,d.aln_data_value")->from('VX_airline a');
		$this->db->join('VX_data_defns d','d.vx_aln_data_defnsID = a.airlineID','LEFT');
	...
	function addFeedBack($data){
		$data['date'] = time();
		$this->db->insert('UP_feedback',$data);
		return $this->db->insert_id();
	}
	
	function avgFeedback(){
		$this->db->select('sum(customer_service)/count(*) avg')->from('UP_feedback');
		$query = $this->db->get();
    ...
    function addBidHistory($data){
       $this->db->insert('UP_bid_history',$data);
       return true;
    }
	
	function getOfferStatus($pnr_ref){
		$this->db->select('ref.offer_status status_no,d.aln_data_value status')->from('UP_offer_ref ref');
		$this->db->join('VX_data_defns d','(d.vx_aln_data_defnsID = ref.offer_status AND d.aln_data_typeID = 20)','LEFT');
	...
	function get_cabins($carrier){
		$this->db->select('acd.*,def.vx_aln_data_defnsID')->from('VX_airline_cabin_def acd');
		$this->db->join('VX_data_defns def','(def.alias = acd.level) and aln_data_typeID = 13','INNER');
    ...
    public function getCarrierCabinImages($airlineID,$cabin){
		$this->db->select('i.image')->from('VX_airline_cabin_map m');
	...
	public function getCarrierCabinVideos($airlineID,$cabin){
		$this->db->select('video_links')->from('VX_airline_cabin_map');
